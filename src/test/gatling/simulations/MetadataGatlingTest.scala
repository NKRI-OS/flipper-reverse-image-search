...
class MetadataGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Metadata entity")
        ...
        .repeat(2) {
            ...
            .post("/api/metadatas")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "name":"SAMPLE_TEXT", "value":"SAMPLE_TEXT", "valueType":"SAMPLE_TEXT", "picture_id":null, "title":"SAMPLE_TEXT", "pictureFile":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_metadata_url")))
            .pause(10)
            .repeat(5) {
